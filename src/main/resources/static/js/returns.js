...
$(document).ready(function() {
    ...
    returnSlipTable = $('#returnSlipTable').DataTable({
        ...
        ajax : {
            method : "GET",
            url : "/return/getFilteredReturnSlips",
            dataSrc : "data",
            data : function (d) {
                let param = {};
                let statusList = $('#returnSlipStatus').val();
        ...
        'columnDefs': [{
            ...
            "render": function(data, type, full, meta) {
                ...
                let selectHtml = '<div class="dropdown bootstrap-select returnStatusSelect">';
                selectHtml += '<select aria-label="Status" class="returnStatusSelector selectpicker" data-container="body" tabindex="null">';
                selectHtml += '<option value="1" ' + (data === 1 ? 'selected' : '') + '>Created</option>';
                selectHtml += '<option value="2" ' + (data === 2 ? 'selected' : '') + '>Finalized</option>';
                selectHtml += '</select>';
                selectHtml += '</div>';
                return selectHtml;
            }
    ...
    $('#returnSlipTable tbody').on('loaded.bs.select changed.bs.select', '.returnStatusSelector', function (e, clickedIndex, isSelected, previousValue) {
    ...
    returnSlipTable.on('click', 'tbody tr', function (event) {
        if (!$(event.target).closest('.returnStatusSelect').length &&
            !$(event.target).closest('.call-checkbox').length &&
            !$(event.target).closest('.editPacking').length) {
            let tr = event.target.closest('tr');
            let row = returnSlipTable.row(tr);
            let idx = detailRows.indexOf(tr.id);

            $.ajax({
                url: "/return/getSalesOrderItemsById",
                method: 'GET',
                data: {
                    returnSlipId: row.data().id
                },
                success: function(response){
    ...
    returnSlipTable.on('draw', () => {
        $('.returnStatusSelector').selectpicker();
    ...
    $('#searchReturnSlipBtn').on('click', function() {
        returnSlipTable.draw();
    });

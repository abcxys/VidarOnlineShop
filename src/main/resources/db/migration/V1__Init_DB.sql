...
create table sales_orders_hardwoodfloors
(
    id          int8 generated by default as identity,
...
alter table if exists sales_orders
    add constraint fk_dealer_id foreign key (dealer_id) references dealers;
alter table if exists sales_orders
    add constraint FK32ql8ubntj5uh44ph9659tiih foreign key (user_id) references users;
alter table if exists sales_orders_hardwoodfloors
    add constraint FK9c3l5vucp0yrrwpc0v8vk8knt foreign key (hardwoodfloors_id) references hardwoodfloors;
alter table if exists sales_orders_hardwoodfloors
    add constraint FK8jft4d30d5dgvauht7ssndwau foreign key (sales_order_id) references sales_orders;
